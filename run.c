...
// #define USE_260K_MODEL
#define USE_15M_MODEL
...
#ifdef USE_260K_MODEL
  INCLUDE_FILE(".rodata", "./stories260K.bin", weights);
#else
  INCLUDE_FILE(".rodata", "./stories15M.bin", weights);
#endif
...
// load the tokenizer data block from the model.bin file
// INCLUDE_FILE(".rodata", "./tok512.bin", tokenizer);
#ifdef USE_260K_MODEL
  INCLUDE_FILE(".rodata", "./tok512.bin", tokenizer);
#else
  INCLUDE_FILE(".rodata", "./tokenizer.bin", tokenizer);
#endif
...
void read_checkpoint_header(Config* config, TransformerWeights* weights,
    ...
    memcpy(config, &weights_data, sizeof(Config));
    int shared_weights = config->vocab_size > 0 ? 1 : 0;
    config->vocab_size = abs(config->vocab_size);
    *file_size = sizeof(Config) + (int)(weights_end - weights_start);

    // map the data pointer to the WEIGHTS array
    *data = (float*)weights_data;
